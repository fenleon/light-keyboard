...
fun Lp3Keyboard(
    ...
    // A platform-agnostic monotonic clock for trail-fade timestamps — pointer event
    // timestamps aren't guaranteed to share a clock domain across Compose targets, so the
    // trail uses this instead of change.uptimeMillis.
    val timeMark = remember { TimeSource.Monotonic.markNow() }
    ...
    // CompositionLocal. lightFontFamily may scan the platform's available fonts,
    // which we don't want to run per-key.
    val akkurat = lightFontFamily()
    ...
    LaunchedEffect(Unit) {
        while (true) {
            ...
            while (trailPoints.isNotEmpty()) {
                ...
                nowMs = timeMark.elapsedNow().inWholeMilliseconds
                ...
                if (swipeConfig != null) {
                    Modifier.pointerInput(swipeConfig) {
                        ...
                        awaitEachGesture {
                            ...
                            // Trail-fade timestamps use timeMark (see above) rather than
                            // change.uptimeMillis, since the two aren't guaranteed to be on
                            // the same clock domain on every Compose target.
                            val pointTimes = ArrayList<Long>()
                            xs.add(down.position.x)
                            ys.add(down.position.y)
                            ts.add(0f)
                            pointTimes.add(timeMark.elapsedNow().inWholeMilliseconds)
                            ...
                            while (true) {
                                ...
                                if (!swipeStarted) {
                                    ...
                                    trailPoints.add(
                                        TrailPoint(p.x, p.y, timeMark.elapsedNow().inWholeMilliseconds)
                                    )
                                }
...
fun RowScope.Key(
    ...
    var pressed by remember { mutableStateOf(false) }
    val label = remember(code) { buildString { appendCodePointCompat(code) } }
    ...
    Box(
        ...
        Text(
            ...
            fontSize = LocalKeyTextSize.current,
            modifier = Modifier.then(
                if (enableKeyAnimation) {
                    Modifier.graphicsLayer {
                        val isPressed = pressed
